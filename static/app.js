...
function renderDetail() {
  ...
  ${review?`<div class="notice">Düşük kanıtlı inceleme adayı. ${esc(i.explanation)}${i.linked_incident_ids.length?`<br>Aidiyeti araştırılan olaylar: ${i.linked_incident_ids.map(esc).join(", ")}`:""}${Object.keys(i.message_targets||{}).length?`<br>Mesaj hedefleri: ${esc(JSON.stringify(i.message_targets))}`:""}</div>`:`<details class="sec" open><summary><span class="number">01</span> KANIT ÖZETİ <span class="sec-hint">${(i.evidence_checks||[]).length} kanıt</span></summary><ul class="evidence-checks">${(i.evidence_checks||[]).map(c=>`<li><span aria-hidden="true">✓</span><div>${esc(c.text)}</div></li>`).join("")}</ul></details>`}
